function Truck_trailer_plot(initial_p,target_p,parameters, info, XY0)
...
plot(initial_p(1), initial_p(2),'rx'); 
hold on;
plot(target_p(1), target_p(2),'gx'); 
% set bounds
xbound = [-50 50];
ybound = [-30 30];
xlim(xbound);
...
boxes = struct('width',{17.5,17.5},'height',{20,20},...
               'position',{[-11.25;-20],[11.25;-20]},...
               'theta',{0,0});
for j = 1:length(boxes)
    obstacle_struct = boxes(j);
    obstacle = collisionBox(obstacle_struct.width,obstacle_struct.height,0);
    obstacle.Pose(1:3,1:3) = eul2rotm([obstacle_struct.theta 0 0]);
    obstacle.Pose(1:2,4) = obstacle_struct.position;
    show(obstacle)
end
...
if nargin <= 3
    ...
    animation(gca, 0, initial_p', [0 0], parameters);
else
    f.Name = ['Automated Parking Animation (x0 = ' num2str(initial_p(1)) ', y0 = ' num2str(initial_p(2)) ')'];
    ...
    animation(gca, tTrackHistory, xTrackHistory, uTrackHistory(:,1), parameters);
...
function animation(ax, tOut, qOut, alpha, params)
